...
def fetch_and_preprocess_data(
    ...
    if os.path.exists(unprocessed_file_path):
        ...
        datasett = openml.datasets.get_dataset(
            dataset_id,
            download_data=True,
    ...
    save_process_data(
        train_df_processed, train_df_original, path, datahandler, test=False
    )
    save_process_data(
        test_df_processed,
        test_df_original,
...
def preprocess_for_json(data):
    ...
    if isinstance(data, dict):
        ...
        return tuple(preprocess_for_json(item) for item in data)
    # 处理所有 NumPy 整数类型
